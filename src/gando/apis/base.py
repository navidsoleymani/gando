...
def set_rollback() -> None:
    ...
    so outer transaction handlers will roll back accordingly.
...
class BaseAPI(APIView):
    ...
    def __paste_to_request_func_loader(
        self, f: str, request, *args, **kwargs) -> Any:
        """
    ...
    def paste_to_request_func_loader_play(self, request, *args, **kwargs):
        ...
        for key, f in SETTINGS.PASTE_TO_REQUEST.items():
            rslt = self.__paste_to_request_func_loader(
    ...
    def handle_exception(self, exc: BaseException) -> Response:
        ...
        if isinstance(exc, EnduserResponseAPIMessage):
            if isinstance(exc, EnduserErrorResponseAPIMessage):
                self.set_status_code(exc.status_code)
                self.add_error_message_to_messenger(
                ...
                self.set_status_code(exc.status_code)
                self.add_warning_message_to_messenger(
                    code=exc.code, message=exc.message)
    ...
    def exception_handler(self, exc: Exception, context: Dict[str, Any]) -> \
        Optional[Response]:
        """
    ...
    def _exception_handler_messages(
        self, msg: Any, base_key: Optional[str] = None) -> None:
        """
    ...
    def _handle_exception_gando_handling_true(
        self, exc: BaseException) -> Response:
        """
    ...
    def _handle_exception_gando_handling_false(
        self, exc: BaseException) -> Response:
        """
    ...
    def finalize_response(
        self, request, response: Response, *args, **kwargs) -> Response:
        """
        ...
        - Calls the parent `finalize_response` to allow DRF to apply any remaining behavior.
        ...
        """
        mock_server_status = self.request.headers.get(
            "Mock-Server-Status") or False
        mock_server_switcher = getattr(self, "mock_server_switcher", False)
        if mock_server_switcher and mock_server_status and hasattr(
            self, "mock_server"):
            # If view is configured to use a mock server, call it and bypass normal finalization.
            return super().finalize_response(
                request, self.mock_server(), *args, **kwargs)
        ...
        if isinstance(response, Response):
            ...
            exception = self.get_exception_status(
    ...
    def _response_validator(self, input_data: Any) -> Any:
        ...
        if isinstance(input_data, list):
            return [self._response_validator(i) for i in
                input_data] if input_data else []
        if isinstance(input_data, dict):
            if not input_data:
                return None
            return {k: self._response_validator(v) for k, v in
                input_data.items()}
        return input_data
    ...
    def response_context(self, data: Any = None) -> Dict[str, Any]:
        ...
        """
        self.response_schema_version = self.request.headers.get(
            "Response-Schema-Version") or "1.0.0"
    ...
    def _response_context_v_1_0_0_response(self, data: Any = None) -> Dict[
        str, Any]:
        """
    ...
    def _response_context_v_2_0_0_response(self, data: Any = None) -> Dict[
        str, Any]:
        """
    ...
    def __add_to_messenger(
        self, *, message: Any, code: Any, type_: str) -> None:
        """
    ...
    def add_fail_message_to_messenger(
        self, *, message: Any, code: Any) -> None:
        self.__add_to_messenger(message=message, code=code, type_="FAIL")

    def add_error_message_to_messenger(
        self, *, message: Any, code: Any) -> None:
        self.__add_to_messenger(message=message, code=code, type_="ERROR")

    def add_warning_message_to_messenger(
        self, *, message: Any, code: Any) -> None:
        self.__add_to_messenger(
            message=message, code=code, type_="WARNING")

    def add_success_message_to_messenger(
        self, *, message: Any, code: Any) -> None:
        self.__add_to_messenger(
            message=message, code=code, type_="SUCCESS")
    ...
    def get_headers(self, value: Optional[Dict[str, Any]] = None) -> Dict[
        str, Any]:
        """
    ...
    def __many(self) -> bool:
        ...
        if (isinstance(self.__data, dict) and
            {"count", "next", "previous", "results"}.issubset(
                self.__data.keys())):
            return True
        return False
    ...
    def __many_v_2_0_0_response(self) -> bool:
        ...
        if isinstance(self.__data, list):
            return True
        if (isinstance(self.__data, dict) and
            {"count", "next", "previous", "result"}.issubset(
                self.__data.keys())):
            return True
        return False
    ...
    def __has_warning(self) -> bool:
        ...
        return bool(
            self.__warnings_message) and self.__warning_message_messenger()
    ...
    def get_content_type(self, value: Optional[str] = None) -> Optional[
        str]:
    ...
    def monitor_play(
        self, monitor: Optional[Dict[str, Any]] = None, *args, **kwargs) -> \
        Dict[str, Any]:
        """
        ...
        for key, f in SETTINGS.MONITOR.items():
            monitor_out[key] = self.__monitor_func_loader(
                f, *args, **kwargs)
        return monitor_out
    ...
    def validate_data_v_2_0_0_response(self) -> Dict[str, Any]:
        ...
        if isinstance(data, list):
            if self.pagination:
                return {
                    "count": len(data), "next": None, "previous": None,
                    "result": data
                }
            return {"result": data}
        ...
        if isinstance(data, dict):
            ...
            if {"count", "next", "previous", "results"}.issubset(
                data.keys()):
                if self.pagination:
                    return {
            ...
            if {"count", "page_size", "page_number", "result"}.issubset(
                data.keys()):
                ...
                if self.pagination:
                    return {
                        "count": data.get("count"), "next": n,
                        "previous": p, "result": data.get("result")
                    }
    ...
    def __get_pagination_url(
        self, *, page_size: int, page_number: int, count: int) -> Tuple[
        ...
        last_page = max(
            1, math.ceil(count / page_size)) if count > 0 else 1
    ...
    def __development_messages_display(self) -> bool:
        ...
        if self.__development_state:
            return self.request.headers.get(
                "Development-Messages-Display", "True") == "True"
        return False
    ...
    def __exception_status_display(self) -> bool:
        ...
        if self.__development_state:
            return self.request.headers.get(
                "Exception-Status-Display", "True") == "True"
        return False
    ...
    def response(self, output_data: Any = None) -> Response:
        ...
        """
        return Response(
            output_data, status=self.get_status_code(),
            headers=self.get_headers())
    ...
    def convert_filename_to_url(self, file_name: Optional[str]) -> \
        Optional[str]:
    ...
    def convert_filename_to_url_localhost(
        self, file_name: Optional[str]) -> Optional[str]:
    ...
    def __auto_default_messenger_message_adder(
        self, status_code, message, prefix_code=''):
        if isinstance(message, list):
            for m in message:
                ...
                if m is None:
                    continue
                self.__default_messenger_message_adder_detector(
                    status_code, m, f'{prefix_code}{m.code}')
        elif isinstance(message, dict):
            for k, v in message.items():
                self.__auto_default_messenger_message_adder(
                    status_code, v, f'{k}__')
        elif message is None:
            self.__default_messenger_message_adder_detector(
                status_code, message)
        else:
            self.__default_messenger_message_adder_detector(
                status_code, message, f'{prefix_code}{message.code}')

    def __default_messenger_message_adder_detector(
        self, status_code, message, code=None):
        if 100 <= status_code < 200:
            self.__add_to_messenger(
    ...
    def __set_default_message(self) -> None:
        ...
        if 100 <= status_code < 200:
            self.set_warning_message(
                "status_code_1xx", self.__default_message())
        elif 200 <= status_code < 300:
            self.set_info_message(
                "status_code_2xx", self.__default_message())
        elif 300 <= status_code < 400:
            self.set_error_message(
                "status_code_3xx", self.__default_message())
        elif 400 <= status_code < 500:
            self.set_error_message(
                "status_code_4xx", self.__default_message())
        elif status_code >= 500:
            self.set_error_message(
                "status_code_5xx", self.__default_message())
        else:
            self.set_error_message(
                "status_code_xxx", self.__default_message())
    ...
    def __set_dynamic_message(self, value: Any) -> Optional[str]:
        ...
        if isinstance(value, (ErrorStringMessage, ErrorDetail)):
            self.set_error_message(
                key=value.code, value=value)  # type: ignore[attr-defined]
            return None
    ...
    def cookie_setter(self, key: str, **kwargs) -> None:
        ...
        """
        self.__cookies_for_set.append(
            self.Cookie(key=key, **kwargs).model_dump())
...
class GenericAPIView(DRFGAPIView, BaseAPI):
    ...
    def _checking_validate_user(self, request, *args, **kwargs) -> None:
        ...
        if self.get_check_validate_user():
            ...
            if not _valid_user(
                user_id=kwargs.get(lookup_field), request=request):
    ...
    def adding_user_id_to_request_data(self) -> None:
        """
        Mutate request.data to include the authenticated user's id under
        the configured field.

        Uses `gando.utils.http.request.request_updater` to produce
        a new request object with updated `data`.
        """
    ...
    def get_for_user(self) -> bool:
        """
        Return whether this view should filter queryset for
        the authenticated user (defaults False).
        """
    ...
    def get_queryset(self):
        ...
        - If you get_for_user() True, filters `queryset.filter(<user_field> = request.user.id)`.
        """
        queryset = super().get_queryset()
        if self.get_for_user():
            return queryset.filter(
                **{
                    self.get_user_field_name_id(): getattr(
...
class DestroyAPIView(GenericAPIView, DRFGDestroyAPIView):
    ...
    def get_soft_delete(
        self, soft_delete: Optional[bool] = None, **kwargs) -> bool:
        """
    ...
    def destroy(self, request, *args, **kwargs) -> Response:
        ...
        self.perform_destroy(
            instance=instance, soft_delete=self.get_soft_delete(**kwargs))
