...
def _get_user_agent_device_key(key):
    from gando.models import UserAgentDeviceModel
...
def _create_user_agent_device_key(request):
    from gando.models import (
        UserAgentDeviceModel, user_agent_device_key_creator)
    from gando.utils.client import UADSchema

    params = UADSchema(request)

    with contextlib.suppress(Exception):
        obj = UserAgentDeviceModel.objects.create(
            **params.model_dump())
        return obj.key
