        ...
        if isinstance(input_data, dict):
            ...
            for k, v in input_data.items():
                ...
                if len(k__split) == 1:
                    ...
                    if len(k_split) == 1:
                        _ = self.__parser(v)
                        v_ = _ if k not in tmp else self.__updater(tmp[k],
                            _)
                        tmp[
                            k] = v_ if k != 'src' else f'{self.__media_url}{v_}'
                    else:
                        _ = self.__parser({k_split[1]: v})
                        v_ = _ if k_split[
                                      0] not in tmp else self.__updater(
                            tmp[k_split[0]], _)
                        tmp[k_split[
                            0]] = v_ if k != 'src' else f'{self.__media_url}{v_}'
                else:
